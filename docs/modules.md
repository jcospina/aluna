...
| 3 | **Opinionated Capability UI** | The capabilities the app builds look and feel like a coherent product — styled lists, a shared modal, a prefilled detail view — not a 1990s form dump | Platform UI components · closed-value design contract · primitive vocabulary · few-shot design gallery · design-lint gate rung · `ui_intent` (detail/modal) · click-to-open | M1–M2 |
...
**Goal:** make the capabilities the app builds **look and feel like a coherent product**, without a developer hand-crafting each one. Today a freshly built capability is born usable but ugly — the builder gets no design guidance, so it reproduces the same bare *[title][empty state][form]* scaffold every time. This module takes the **structural chrome into the platform** (a shared modal, the list scaffolding, a spec-rendered form) and hands the builder a **closed-value design contract + a few-shot exemplar gallery + a design-lint gate rung**, so every capability is consistent, on-brand, and pleasant — while the *list item* stays the builder's creative surface and varies to fit the data.
...
- **3.1 — Platform UI components (the thick shell).** The shared, platform-owned **presentational** components generated views plug into: the **modal** (open/close, focus handling — the one component every capability reuses), the **list scaffolding** (container, empty state, the "New X" button), and the **spec-rendered form** (one labeled control per field by type, all HTMX wiring, close-on-success). Presentation only, no business logic — the same line ARCH §7 already draws for the `data_query` auto-table. Consequently `list.html` and `create.html` stop being generated artifacts. (ARCH §6.1, §7; [ADR-0005](adr/0005-opinionated-capability-ui-design-contract-and-gate.md))
- **3.2 — Closed-value design contract + primitive vocabulary.** Promote the token layer into a **closed** contract the builder must target: generated markup references design tokens only — never raw color/space/font values — plus a small set of **primitive helper classes** (truncation, media-frame, grid/stack). The builder looks tokens up; it never invents them. (CONTEXT.md voice; [design-system.md](design-system.md))
- **3.3 — Few-shot design gallery.** A curated, **repo-only** set of 2–3 deliberately *different* list-item exemplars (e.g. text-forward cards, a media grid, compact metadata rows), each obeying the *same* contract but composing differently, injected into the unit prompt with explicit *"vary, don't copy"* framing. LLM-facing only — never rendered to the user. (ADR-0005)
- **3.4 — The list item as the creative surface.** Re-cut the artifact contract: with `list.html`/`create.html` now platform-owned, `read.ts` (mirrored by `create.ts`) renders the per-item **row** — the one place per-capability visual variation lives — and each row carries its full record as an escaped `data-item` payload plus a click-to-open hook. Amends [ADR-0004](adr/0004-capability-artifact-contract-and-validation-isolation.md); see [ADR-0005](adr/0005-opinionated-capability-ui-design-contract-and-gate.md).
- **3.5 — Design-lint gate rung.** A new **fail-closed** rung in the existing layered gate: reject fabricated tokens / raw values, unknown classes, and rows missing a parseable `data-item` or click hook — fed back through the same bounded fix loop as the type-check rung. This is the enforcement leg (the industry's audit-script / detector-rule pattern) that keeps the contract from being merely advisory. (ARCH §6.2 gate; ADR-0005)
- **3.6 — Detail modal + click-to-open (read-only).** Grow `ui_intent` to the shape ARCH §6.3 already drew — `detail` (which fields the modal surfaces) + `modal: true` — and wire it: clicking a list item opens the platform modal **prefilled** from the row's embedded record, so full content shows even when the row truncates. Read-only in M3; M4's `update` adds the Save button. (ARCH §6.3 "Capability Registry")

### Verify by running it
Type *"I want to keep track of my notes."* → the **Notes** capability builds with a styled list (cards, long text truncated) and a **New note** button — no bare form dump → click *New note* → the **shared modal** opens with an on-brand form → add a note → it appears as a styled card → click the card → the **same modal** opens **prefilled** with the full note (read-only). Then build a differently-shaped capability — *"save links with a title and a url"* — and confirm its list presents **differently** from Notes (not a clone) while reusing the same modal, tokens, and components. Finally, confirm a build that emits a raw color value or a row missing its `data-item` payload **fails the gate** (friendly message, nothing committed).

### Exit criteria
Generated capabilities present through platform-owned components (modal, list scaffolding, spec-rendered form) against a closed-value design contract that the gate enforces; the **list item is the builder's creative surface** and visibly varies by capability; clicking an item opens a **prefilled, read-only** modal. The presentation contract every later module inherits is in place.
...
- **4.1 — Complete the tool set.** Generate `update`, `delete`, `search` handlers. Editing is the **Save button on Module 3's detail modal** (the modal flips from read-only to editable); add the delete affordance and the search/list views. The capability now exposes all five declared tools. (ARCH §6.2 router actions)
...
- **4.3 — Diff engine.** Compute the minimal change between old and new spec at **unit granularity** (handler files, and — under the M3 contract — the row-rendering surface). Regenerate only affected units; feed the AI the previous version of that unit + the spec diff to apply the delta and preserve the rest; stream only changed fragments with targeted `hx-swap`. (ARCH §6.2 "Diff Engine")
- **4.4 — Additive schema evolution.** Migrations that `ADD COLUMN` or use the JSON escape hatch, or **soft-hide** — never `DROP`/destructive rename. Version-namespaced artifacts (`v<n>/`). (ARCH §3, §6.3 "Data Tables", §9.3)
...
**Why sixth:** files are the last user-facing surface of the explicit loop. They reuse the data tool (for the reference), the router (for upload), and record lifecycle (for deletion) — all of which now exist. The list grid and the detail modal from Module 3 are exactly where uploaded images present.
...
- **6.2 — `file` / `file[]` field type.** Schema support for file fields; the data table stores only a **reference** (key + mime + size + original name), never the bytes. A `photos` capability is just a normal capability with a `file` field. (ARCH §6.3 "Capability Registry", §7 "Files")
...
### Verify by running it
Type *"let me save photos with a caption."* → a photos capability with an upload form builds → upload an image → it renders in the list (as a media-grid item, via `/files/<key>`) → click it → the detail modal shows the full image and caption → delete the record → the underlying file is gone.
...
- **7.1 — Define the implicit UX (open design).** Decide how implicit work reaches the user — interrupt vs. quiet background build, where a proposal appears, a persistent assistant presence vs. a transient notice, the tone and timing of notifications. **This is genuinely undefined today: it is design work, not a foregone conclusion**, and it constrains every epic below. The only thing fixed going in is the contract — nothing builds without an explicit confirmation. (ARCH §8 "Loop 2")
...
- **8.1 — Metrics querying.** Latency breakdowns (spec-gen, code-gen, HTML-gen, test-gen, migration, test-run, total wall-clock), model, token counts, success/failure and per-rung gate outcomes per generation. Includes the **behavioral-tier comparison**: rung on vs. off — what it costs in wall-clock and how much drift it actually catches. (ARCH §6.3 "Generation Metrics", §9.6, §6.2)
...
| **Closed-value design contract + design gate** | M3 | Generated markup targets platform components + design tokens only (no raw values); a fail-closed design-lint rung enforces it; structural chrome is platform-owned presentation | §6.2, §6.3, §7, §9.7 |
...
M2 Explicit I  ──────────────────────────────┐  builds the shared backbone:
   ...
   ▼                                          │  build queue, metrics
M3 Opinionated UI                             │
   │  (platform components · closed-value     │  ← presentation contract
   │   contract · few-shot gallery ·          │     reused by every later
   │   design gate · detail modal)            │     user-facing surface
   ▼                                          │
M4 Explicit II                                │
