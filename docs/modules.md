...
- **The implicit-loop UX.** The modules below define the implicit loop's backstage: event capture, the gate, async inference, confirmations, and the hand-off to the build pipeline. They do not define how implicit work reaches the user — whether a proposal interrupts or builds quietly in the background, where it appears, whether there is a persistent assistant presence or a one-off notice, its tone or its timing. Module 7 owns that design question and settles it when it is built. One commitment is fixed already: nothing is built without an explicit confirmation.
...
capability registry, data-access seam, router, orchestrator, AI call and
toolbar — enters inside the first feature that needs it, and everything after
either reuses it or deliberately deepens it.
...
coordinator, and toolbar. Modules 2–6 build all of it during the explicit phase,
...
The explicit loop is complete at the end of Module 6. Module 7 turns on the second loop; Module 8 makes the experiment legible.
...
answer. It needs populated capabilities, which now exist.
...
- **5.3 — Generic auto-table renderer.** A platform-owned, presentational-only table for arbitrary result sets, so it introduces no platform business logic. (ARCH §7 "`data_query`")
- **5.4 — Cheap reject/route classifier.** A friendly refusal for obvious non-queries ("delete everything"). It routes and rejects early; the write restriction itself lives in the supplied read-only adapter and static contract from 5.1, never here. (ARCH §7 "Reads")
...
- **6.1 — Object store (S3-shaped tool).** `put / get / delete / url`, default-backed by the local filesystem (`Bun.file` / `Bun.write`), addressed by opaque key under `storage/<key>`; swappable to R2/S3/Garage by config. This is platform infrastructure: the AI never builds storage. (ARCH §6.3 "Object Store", §7 "Files")
...
**Why seventh:** this thin layer is the whole difference between implicit and explicit. It needs a complete, populated app to observe (Modules 2–6) and reuses the entire build pipeline, adding exactly the two things explicit never needed: full-fidelity event capture and the behavior→proposal classifier path.
...
- **7.6 — Proposal + decision (contract fixed, presentation per 7.1).** Confirm
  ...
  Presentation comes from 7.1. (ARCH §8 "Loop 2", §9.3)
...
- **8.3 — Experimenter surface.** An internal view/report to read the dataset, deliberately outside the user-facing product voice — the friendly app shows no internals. (ARCH §9.7)
