...
*system* did to build itself (distinct from the event log, which is M10's record
...
- **Columns per the PLAN's step 8**: timing breakdown (spec-gen, code-gen,
  ...
  The test-gen/test-run columns are what let M11 quantify the behavioral tier
...
- [x] Writes go through the read-write connection; querying (M11's future surface)
