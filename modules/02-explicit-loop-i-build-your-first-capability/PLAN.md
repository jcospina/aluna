...
6. **Resolver: full enum from day one, act on one.** The intent schema speaks
   ...
   streamed over the same per-build channel and logged to metrics — so M4/M6
...
8. **Spec surface: a deliberately tiny pantry.** The AI authors the **spec**
   ...
   Excluded for now: list types (M4 evolution), `file`/`file[]` (M7),
   ...
   M6's NL→SQL catalog), and `extra` (the JSON escape-hatch column, present
