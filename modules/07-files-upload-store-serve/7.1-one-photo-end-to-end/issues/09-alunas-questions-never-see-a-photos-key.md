...
**Two layers, not one.** The issue asked for a scrub where rows become model text. The first
...
**Layer 1: the worker's views** (`src/runtime/query/query-worker-thread.ts`). The question
...
Every catalog table is shadowed by a temp view named like it, listing exactly the columns its
spec knows (`catalogShadow` in `whole-catalog-read-scope.ts`, inactive fields included), each
...
- A file column becomes `json_object` of `kind`, `mime`, `size` and `name` only, or `NULL` when
  the stored value is not one well-formed object with a single `key`. Any of the four that may
  hold an address is withheld; a file named after a key, the way a browser names a file saved
  from `/files/<key>`, keeps its kind, type and size and loses its name.
- A text column, and a list column as a one-element JSON array, shows the withheld phrase when
...
**Layer 2: the scrub of what comes back** (`src/runtime/query/question-file-scrub.ts`).
`renderQuestionRows` (`question-payload.ts`) is still the one place rows become model text, for
the turn's prompt and the answer's, cached per result. It replaces any value that is or contains
