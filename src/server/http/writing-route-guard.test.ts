...
const STREAM_LIMIT = 200_000;

const GUARDS: readonly [string, number, () => MiddlewareHandler][] = [
  ["buffered", TEXT_BODY_LIMIT_BYTES, guardWritingRoute],
  ["streaming", STREAM_LIMIT, () => guardStreamingRoute(STREAM_LIMIT)],
];
...
describe.each(GUARDS)("a %s writing-route guard", (_kind, limit, guardFor) => {
  test("refuses another site's request before it reads a byte of the body", async () => {
    const { app, reached } = guardedEcho(guardFor());
  ...
  test("admits the desk's own request, a navigation and a client that is not a browser", async () => {
    const { app } = guardedEcho(guardFor());
...
describe("a buffered writing-route guard", () => {
  ...
  test("answers a client that hung up mid-body quietly, and a real read failure loudly", async () => {
    const { app, reached } = guardedEcho(guardWritingRoute());
...
describe("a streaming writing-route guard", () => {
  test("hands the body over as it arrives, without holding it", async () => {
    const app = new Hono();
    const body = probeBody(STREAM_LIMIT, "x=");
    let pulledBeforeReading = -1;
    app.post("/write", guardStreamingRoute(STREAM_LIMIT), async (c) => {
    ...
    expect(answer.pulledAtFirstChunk).toBeLessThan(STREAM_LIMIT);
  });
  ...
  test("raises BodyTooLargeError to a route that reads past the limit", async () => {
    ...
    const response = await app.request("/write", streamedInit("POST", probeBody(STREAM_LIMIT + 1)));
