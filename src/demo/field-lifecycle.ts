...
const SEARCH_HANDLER = `export default async function search({ input, query, present }: CapabilityContext): Promise<string> {
  ...
  return query.records({
    sql: 'WITH "search_terms" AS (SELECT "value" AS "term" FROM json_each(?)) SELECT "target"."id" AS "target_id" FROM "cap_field_lifecycle_demo" AS "target" WHERE NOT EXISTS (SELECT 1 FROM "search_terms" AS "search_term" WHERE NOT ((coalesce(instr(platform_search_normalize("target"."entry"), platform_search_normalize("search_term"."term")), 0) > 0) OR (coalesce(instr(platform_search_normalize("target"."reflection"), platform_search_normalize("search_term"."term")), 0) > 0) OR EXISTS (SELECT 1 FROM json_each("target"."tags") AS "tag" WHERE coalesce(instr(platform_search_normalize("tag"."value"), platform_search_normalize("search_term"."term")), 0) > 0) OR EXISTS (SELECT 1 FROM json_each("target"."aliases") AS "alias" WHERE coalesce(instr(platform_search_normalize("alias"."value"), platform_search_normalize("search_term"."term")), 0) > 0))) ORDER BY "target"."created_at" DESC, "target"."id" DESC',
