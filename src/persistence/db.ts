// The platform's SQLite foundation.4: two connections against
// the single database file.
//
// The whole data model collapses to one line: *mutation is constrained
...
//                    on the read path regardless of what SQL is issued:
...
// them; the platform-owned migrations runner builds on `db`.
...
export function openDatabase(path = DB_PATH): PlatformDatabase {
  ...
  // — readers see the last committed snapshot without blocking the
  ...
  // deterministic safety boundary the read path relies on.
