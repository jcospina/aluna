...
/**
 * The same committed v1, published tier-on — the only base an evolution can carry a frozen suite
 * from. Its setup rows predate any added column, so the Gate sees a historical `null` in one.
 */
...
/**
 * Publishes and activates the committed v1 and seeds the one historical record. `spec` may bring
 * another shape if it keeps the `notes` id, and its gate must have been issued for that spec.
 */
...
/**
 * Answers whatever the engine asks, by reading the prompt rather than the call order: a case
 * proving "these units were copied" must not depend on the test knowing the request order.
 */
export function engineProvider(
  ...
  // A queued override answers successive generations of one unit in order, then falls back to
  // the good default: the build writes bad bytes and the repair regeneration writes correct ones.
...
> & {
  ...
  /**
   * Replaces one unit's generated bytes. A string answers every generation; an array answers them
   * in order then reverts to the good default, which exercises a repair rather than a dead end.
   */
  readonly unitOverrides?: Readonly<Record<string, UnitOverride>>;
  /**
   * Writes the lifecycle through the real SQLite recorder, not the in-memory stub. A case faulting
   * inside the activation transaction needs it: a Map cannot roll back, and would report success.
   */
...
/**
 * The durable lifecycle row as SQLite holds it: the only reading that survives a rolled-back
 * activation transaction, and readable after a throw when `evolve` returned nothing.
 */
...
/**
 * The tracer's candidate: "add a due date to my notes and make it stand out in the list". A
 * multi-fact evolution whose free-text `behavior` change pulls in decision 22's Handler fallback.
 */
...
/**
 * The three places one "nothing had to be re-proven" verdict must agree: the Gate's run, the
 * snapshot's tier metadata, and the metrics stage vector. One alone is not a trustworthy verdict.
 */
