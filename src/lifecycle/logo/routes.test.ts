...
describe("the attempt route", () => {
  ...
  // `HX-Request` is a custom header, so a cross-origin request carrying it needs a CORS preflight
  // this route never answers. Without it, any visited page could burn three ~$0.08 attempts.
  ...
  test("replaces only the face — nothing else comes back with it", async () => {
    ...
    // The button and nothing around it: this is the one swap on the desk nobody asked for, and the
    // menu and rename editor are the user's own state; a picture must not eat a name (5.9/01).
...
describe("the logo route", () => {
  ...
  // The stored bytes are never touched (L8), so three headers are the whole inertness guarantee.
  // Equality, not containment: `createApp`'s blanket CSP must not relax this route's stricter one.
  ...
  test("marks a present incarnation's artwork immutable", async () => {
    ...
    // Safe only because of L7 and the incarnation in the path: these exact bytes are never remade,
    // and the one thing that changes a picture mints an address sharing no cache entry.
...
describe("the logo route, when there is nothing to serve", () => {
  ...
  // Only bytes-on-disk prove the registry state is the gate: these two put a readable drawing at
  // the path and still need a 404. `insertCapability` writes no logo lifecycle — use the writers.
  test("a generating incarnation is refused even though its bytes are on disk", async () => {
    install(conns, notesRow());
    // Exactly what a claim whose process died between install and finalize leaves: the file
    // installed, the row still `generating`. Recovering it is recovery's job, not this route's.
  ...
  test("a present row whose file has gone missing serves nothing", async () => {
    install(conns, notesRow());
    await appWith(drawing).request(ATTEMPT_PATH, ATTEMPT);
    // The row says `present`; the bytes are gone. Reconciling it is desk-load recovery's job.
    // the route's is to not pretend, and to not let a browser cache the gap.
    rmSync(storedLogo());
...
// The immutable directive is only as good as the page that names the address: a stale desk would
// go on asking for a deleted lifetime's picture, answered from cache without the server hearing.
...
describe("the logo route, across a delete and a rebuild", () => {
  ...
  test("a rebuilt semantic id gets a different address the old cache entry cannot answer", async () => {
    ...
    // a year has nothing to hand back, which is why the incarnation is in the path.
