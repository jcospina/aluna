...
//
// Module 10 installs the store for real; until then a platform without one purges nothing and
// reports zeroes, while the M10 seam fake installs this fixed shape so the purge is proven now.
//
...
/** The fixed installed Event Log store M10 will own; the M4 seam fake installs the same shape. */
export {
  EVENT_LOG_OWNERSHIP_TABLE,
  EVENT_LOG_TABLE,
} from "../../platform/persistence/table-names.ts";
