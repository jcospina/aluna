...
export function buildFullBehavioralTestPrompt(spec: CapabilitySpec): string {
  ...
  return [
    ...
    "- `expectFragmentIncludes`, `expectFragmentExcludes`, and `expectFragmentIncludesInOrder`: success-fragment assertions; use empty arrays when not needed. Ordered assertions belong only to read/search; create/update/delete must leave `expectFragmentIncludesInOrder` empty. Search ordering evidence belongs there; platform smoke owns match-set/filtering evidence.",
    ...
    "- Success fragment assertions may use only Action-relevant synthetic values: create/update use submitted input values, or values from `expectedRows` only when it contains exactly one affected mutated row; read/search use setupRows or expectedRows; delete uses none. Never assert generated product copy.",
    ...
    "- Create/update return one mutated item, never the refreshed collection: unrelated preserved rows may be proved through `expectedRowCount`, but must not appear in fragment assertions. Every mutation fragment marker must be absent from unrelated setup rows, even when it is also a submitted value. When asserting a transformed returned value, keep `expectedRows` to the one affected row. Leave ordered assertions empty, and prefer all fragment arrays empty when state proves the behavior. Delete must leave every fragment assertion array empty. Read should include seeded record values; a search with searchable fields must use ordered markers from matching rows and may leave exclusions empty.",
