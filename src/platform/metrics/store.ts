...
// log, which is M10's record of what the user did. Every build, every failed build and every
...
// build fills only what it reached. `testGenMs` and `testRunMs` are what M11 weighs the tier by.
 ...
 * ride the read-only connection by convention — the M11 query surface.
 */
 ...
 * Ordered deterministically so M11's queries see a stable order.
 */
