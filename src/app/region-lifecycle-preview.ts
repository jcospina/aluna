...
//
// The same two numbers make the *other* invisible timing watchable: a deletion's drain.
// The drain deadline sits above the longest a single handler may run, so a slow read now
// delays a deletion instead of failing it — but nothing about a five- or ten-second wait
// is visible from a page that answers in milliseconds either. So the preview offers to
// drain its own gate and reports how long the wait actually took.
//
// Its read gate is its own coordinator over a synthetic incarnation, so the preview never
// touches the registry's. Nothing here is ever finalized: a successful drain reopens the
// gate, because the preview has to survive being used twice. Scaffolding: it comes down
// when the window ships the same rule on a real capability.
...
export type PreviewDrainOutcome = "already_closing" | "deletion_drain_timeout" | "drained";

export interface PreviewDrainReport {
  readonly outcome: PreviewDrainOutcome;
  readonly waitedMs: number;
  /** The deadline actually in force, so the page never restates the number itself. */
  readonly deadlineMs: number;
  /** What the deadline used to be, which is the whole reason this readout exists. */
  readonly previousDeadlineMs: number;
}

/** The deadline this preview exists to show the drain waiting past. */
const SUPERSEDED_DRAIN_TIMEOUT_MS = 5_000;

function drainReport(outcome: PreviewDrainOutcome, waitedMs: number): PreviewDrainReport {
  return {
    outcome,
    waitedMs,
    deadlineMs: DEFAULT_READ_DRAIN_TIMEOUT_MS,
    previousDeadlineMs: SUPERSEDED_DRAIN_TIMEOUT_MS,
  };
}
...
export function registerRegionLifecyclePreviewRoutes(app: Hono): RegionLifecyclePreviewSurface {
  ...
  app.get(`${REGION_LIFECYCLE_PREVIEW_ROUTE}/read`, async (c) => {
    ...
    try {
      ...
      // A closing gate admits no new readers. This is the real route's refusal — the same
      // bytes and the same 409 a capability read answers with, not a stand-in — and here
      // it is on screen long enough to read.
  ...
  // The drain, at the production deadline. It closes the gate, waits for whatever is
  // still reading, and reopens — the preview is never destroyed, so `finalizeClose` is
  // deliberately absent. The elapsed time is the point of the route.
  app.post(`${REGION_LIFECYCLE_PREVIEW_ROUTE}/drain`, async (c) => {
    const startedAt = Date.now();
    try {
      const lease = await readGates.closeAndDrain(PREVIEW_INCARNATION);
      readGates.reopen(lease);
      return c.json(drainReport("drained", Date.now() - startedAt), 200, {
        "cache-control": "no-store",
      });
    } catch (error) {
      if (error instanceof ReadGateDrainTimeoutError) {
        return c.json(drainReport("deletion_drain_timeout", Date.now() - startedAt), 200, {
          "cache-control": "no-store",
        });
      }
      if (error instanceof ReadGateUnavailableError) {
        return c.json(drainReport("already_closing", Date.now() - startedAt), 200, {
          "cache-control": "no-store",
        });
      }
      throw error;
...
<html lang="en">
  ...
  <body>
    <p class="preview-banner">
      Developer preview, and it speaks in engineering terms rather than product voice —
      read tokens, handler deadlines, gate states. The release rule itself ships in the
      shell; this page only makes it slow enough to watch. Its read gate is a preview
      coordinator over a synthetic incarnation, so nothing here touches the registry and
    ...
    <h2 class="preview-title">A deletion waits for the reader</h2>
    <p class="preview-view">
      Start a read that holds for longer than five seconds, then drain the gate while it is
      still running. The gate goes to <code>closing</code>, refuses new reads, and waits —
      past the old five-second deadline — until the reader finishes on its own. The drain
      deadline is ${DEFAULT_READ_DRAIN_TIMEOUT_MS}ms; it used to be
      ${SUPERSEDED_DRAIN_TIMEOUT_MS}ms, which is less than the
      ${DEFAULT_CAPABILITY_HANDLER_TIMEOUT_MS}ms a single handler may run, so this same
      wait used to refuse the deletion instead.
    </p>
    <p class="preview-view">
      Nothing is deleted here. This drains the preview's own gate and reopens it — the
      waiting half of a real deletion, without the destructive half.
    </p>

    <div class="preview-controls">
      <button type="button" class="btn btn--danger" data-preview-drain>Drain the gate</button>
      <span class="preview-view" data-preview-drain-status aria-live="polite">Not draining.</span>
    </div>
